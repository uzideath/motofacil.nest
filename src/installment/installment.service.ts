...
export class InstallmentService {
    ...
    async create(dto: CreateInstallmentDto) {
        ...
        if (loan.status === LoanStatus.COMPLETED || loan.status === LoanStatus.DEFAULTED) {
            throw new BadRequestException(`Loan status is ${loan.status}. No more payments allowed.`);
        }
        ...
        const newStatus =
            updatedRemaining <= 0 && updatedDebt <= 0 ? LoanStatus.COMPLETED : LoanStatus.ACTIVE;
